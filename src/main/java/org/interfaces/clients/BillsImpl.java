...
public class BillsImpl extends ClientsImpl implements Bills {
    ...
    public void proccesBuy(String identification) {
        ...
        inputBuys(identification);
    }


    //////REGISTRAR LAS COMPRAS
    public void inputBuys(String identification) {
        boolean state = true;
        ...
        if (option.equalsIgnoreCase("SI")) {
            saveHeaderBilssd(listCar, identification);
            printHeader();
    ...
    void saveHeaderBilssd(List<Buy> car, String ident) {
        try {
            ...
            preparedStatement1.setString(3, ident);
            preparedStatement1.executeUpdate();
        } catch (SQLException e) {
            throw new ExceptionCompras("ERROR AL REGISTRAR LA COMPRA");
        }
    }
    ...
    void saveDetailBills(double priceTotal, int idProduct, int idCabecera) {
        ...
        try {
            Connection connection = getConnection();
            String sql = "insert into db_factura.detalle_factura (PRECIO_TOTAL, ID_PRODUCTO ,id_cabecera) VALUES(?,?,?) ";
    ...
    int maxIndexHeader() {
        ...
        try {
            Connection connection = getConnection();
            String sql = "SELECT max(ID+1) as MAX FROM db_factura.cabecera_factura;";
            PreparedStatement preparedStatement = connection.prepareStatement(sql);
            ResultSet result = preparedStatement.executeQuery();
            while (result.next()) {
